...
import { Share2, XCircle, FileText, Download } from "lucide-react";
...
function getCleanFileName(name: string): string {
  ...
  // Remove UUID prefix if present e.g. "ff101cfe-d74e-4e17-9673-e9604b246fa5-Interview-Invitation-Letter.pdf"
  return name.replace(/^[0-9a-fA-F-]{32,38}-/, "");
}

export default function Documents() {
  const { docRequests, removeDocRequestAsync, files, toast } = useStore();
        ...
        {tab === "Requests" ? (
          ...
          files.length ? (
                      ...
                      <Td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <a
                            ...
                            className="font-medium text-foreground text-[13px] hover:text-foreground/80 hover:underline transition-colors"
                          >
                            {cleanName}
                          </a>
                        </div>
            ...
            <EmptyState
              ...
              hint="When clients upload files through document request links, they will appear here."
              action={
                <Button variant="primary" onClick={() => setOpenRequestModal(true)}>
                  + Request documents
                </Button>
